      ...
      <div className="bg-white border border-trust-border rounded-xl p-6 shadow-trust max-w-2xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-trust-muted">Resuelve captchas automaticamente al iniciar sesion</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {status === 'up-to-date' && (
          <span className="text-xs text-trust-green flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-trust-green" />Ya tienes la última versión
          </span>
        )}
        {status === 'available' && (
        ...
        {status === 'error' && (
          <span className="text-xs text-red-500" title={errorMsg}>Error: {errorMsg.slice(0, 60)}</span>
        )}
      </div>
    </div>
  );
